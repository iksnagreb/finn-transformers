...
def test_data(context, batch_size, input_info, output_info):
    ...
    if len(input_info) > 2:
        device_token_type_tensor = next(iter(device_token_type.values()))
    else:
        device_token_type_tensor = None

    return device_input, device_outputs, device_attention_mask, device_token_type_tensor, stream_ptr, torch_stream
...
def run_inference(context, test_loader, device_outputs, device_attention_mask, device_token_type, stream_ptr, torch_stream, batch_size=1, input_info=None, output_info=None, accuracy_flag=False):
    """
